...
static struct task_struct *pcur_task = &init_task;

static struct task_struct *task[NR_TASKS] = { &init_task, };
	...
	while (1){
		...
		for (i = 1; i < NR_TASKS; i++){
			if (task[i] && task[i]->status != THREAD_EXIT){
				...
				schedule ();
				break;
				//continue;//debugging only(这行有歧义, 忽略)
			}
		}
		if (!remain){
			break;
...
void schedule (void)
{
	...
	if (next){
		switch_to (pcur_task, next);//汇编语言switch_to.s 定义的函数 (不同平台需要改不同的汇编代码)
	}
}

void th_sleep (int seconds)
{
	pcur_task->wakeuptime = get_time_ms () + 1000 * seconds;
	pcur_task->status = THREAD_SLEEP;
	schedule ();
}

void do_timer (void)
{
	if (--pcur_task->counter > 0){
		return;
	}
	pcur_task->counter = 0;
	schedule ();
...
__attribute__((constructor)) static void init ()
{
	...
	if(signal(SIGALRM, do_timer) == SIG_ERR){
		perror("signal()");
		return;
	}
}
...
//c 语言版本的switch_to() 代码
void switch_to (struct task_struct *pcur_task, struct task_struct *next){
	closealarm();
	pcur_task = next;//应该是这样做交换的, 这里是我猜的
	openalarm();
	return ;
}

//汇编码switch_to() 是错误的, 什么东西?
//extern void switch_to (struct task_struct *pcur_task, struct task_struct *next);
